...
function DropdownSelect(props) {
    return (
        <select className={`DropdownSelect ${props.className}`} value={props.value} onChange={(e) => props.onChange(e.target.value)} style={props.icon ? {backgroundImage: `url(${props.icon})`, paddingLeft: '32px'} : null}>
